...
function timedCount() {
	...
	for (var i = 1; i < drex_node_times.length; i++) {
		if (position < drex_node_times[i]) continue;
		var next = drex_node_times.length;
		for (var j = i + 1; j < drex_node_times.length; j++) {
			if (drex_node_times[j] == drex_node_times[i]) continue;
			next = j;
			break;
		}
		if (next == drex_node_times.length || position < drex_node_times[next]) {
...
function equalString(str1, str2) {
	...
	for (var i = 0; i < str1.length; i++) {
		...
		if (str1[i] == '_' || str2[i] == '_') {
			var ch = str1[i] == '_' ? str2[i] : str1[i];
			if (ch == ':' || ch == ',') continue;
		}
		if (str1[i] == '-' || str2[i] == '-') {
			var ch = str1[i] == '-' ? str2[i] : str1[i];
			if (ch == ' ' || ch == ',') continue;
		}
		match = false;
		break;
...
function getMainTimeIndex(time) {
	for (var i = 1; i < drex_node_times.length; i++) {
		if (drex_node_times[i] == time) return i;
	}
	return 0;
}

function getPageIndex(pagename) {
	for (var i = 1; i < drex_node_links.length; i++) {
		if (equalString(pagename, drex_node_links[i])) return i;
	}
	var str1 = pagename.substring(0, pagename.length - 5);
	for (var i = 1; i < drex_node_names.length; i++) {
		if (!equalString(str1, drex_node_names[i])) continue;
		var index = getMainTimeIndex(drex_node_times[i]);
		if (index > 0) return index;
	}
	return 0;
}

function initMenuTime() {
	vjwTutorial = (window['drex_file_type'] !== undefined && window.drex_file_type == "vomedia");
	for (var i in drex_node_names) {
		drex_node_times[i] = -1;
		var n1 = drex_node_links[i].indexOf("-");
		var n2 = drex_node_links[i].indexOf("_");
		if (n1 == 5 && n2 == 2) {
			var v1 = parseInt(drex_node_links[i].substring(0, 2));
			var v2 = parseInt(drex_node_links[i].substring(3, 5));
			var v3 = v1 * 60 + v2;
			if (v3 == 0 && i > 1) drex_node_times[i] = drex_node_times[i - 1];
			else drex_node_times[i] = v3;
		} else if (i > 1) drex_node_times[i] = drex_node_times[i - 1];
	}
	if (vjwTutorial) timedCount();
}
SAN_WHOLE.namespace('SAN_WHOLE.dataManager');
SAN_WHOLE.dataManager = (function () {
	var data_resize = SAN_WHOLE.data_resize;
	var data_search = SAN_WHOLE.data_search;
	var data_menu = SAN_WHOLE.data_menu;
	drex_node_names = data_menu.DREX_NODE_NAMES;
	drex_node_links = data_menu.DREX_NODE_LINKS;
	drex_node_paths = data_menu.DREX_NODE_PATHS;
	drex_node_times = new Array(data_menu.DREX_NODE_LINKS.length);
	...
	var data_index = SAN_WHOLE.data_index;
	...
	var API = {
		...
		isFrameModeEnabled: function () {
			if (data_resize.SANWHOLE_FIT_HEIGHT_TO_WINDOW === 1) {
				return true;
			} else {
				return false;
			}
		...
		getSearchTextNoResults: function () {
			return data_search.SANWHOLE_NOT_FOUND;
		},

		getSearchTextInPreviewMode: function () {
			return data_search.SANWHOLE_PREVIEW_MODE_SEARCH_IS_DISABLED_NOTICE;
		},

		getErrorInLocalSearch: function () {
			return data_search.SANWHOLE_ERROR_LOCAL_SEARCH;
		},
		getSearchTextEmptyString: function () {
			return data_search.SANWHOLE_EMPTY_STRING;
		},
		getSearchTextInProgress: function () {
			return data_search.SANWHOLE_IN_PROGRESS;
		},
...
SAN_WHOLE.namespace('SAN_WHOLE.dom');
SAN_WHOLE.dom = {
...
SAN_WHOLE.namespace('SAN_WHOLE.searchEngine');
SAN_WHOLE.searchEngine = (function () {
	...
	function ID() {
		...
		var sID = dirname() + "/SanwholeData/ids.txt";
	...
	function SearchForNextString() {
		...
		var sURL = dirname() + "/SanwholeData/" + StringPairArray[iStringToSearch][1];
	...
	function GetIndex() {
		...
		var sURL = dirname() + "/SanwholeData/prefixes.txt";
		request.open("GET", sURL);
		request.onreadystatechange = function () {
			if (request.readyState == 4)
				if (request.status == 200 || request.status == 0) {
					var arPrefixes = (request.responseText.toLowerCase()).split(/\s*;\s*/);
					var j = 0;
					SearchResults = new Array();
					var strsearch = strSearchLine.toLowerCase();
					for (var i = 0; i + 2 < arPrefixes.length; i += 3) {
						var n = arPrefixes[i + 2].indexOf(strsearch);
						if (n != -1) {
							SearchResults.push(new Array());
							SearchResults[j].push(arPrefixes[i + 0]);
							SearchResults[j].push(arPrefixes[i + 1]);
							SearchResults[j].push(arPrefixes[i + 2]);
							++j;
						}
					}
					getSearchResultOutput();
	...
	function dirname() {
		...
		var l = window['drex_file_path'];//N2201011758
		while (true) {
			var n = l.lastIndexOf('/');
			if (n == -1) break;
			l = l.substring(0, n);
...
SAN_WHOLE.namespace('SAN_WHOLE.searchManager');
SAN_WHOLE.searchManager = (function () {
	var _class = {
		elementsArr: null,
		isFirstSearch: true,
		dom: null,
		...
		doSetHighlightManager: function () {
			this.highlightManager = SAN_WHOLE.highlightManager;
		},
		...
		doSetNavTreeSearch: function () {
			this.navTreeSearch = SAN_WHOLE.navTree_Search;
		},
		doSetDom: function () {
			this.dom = SAN_WHOLE.dom;
			this.elementsArr = [{
		...
		doSetUrlEncoder: function () {
			var that = this;
			this.urlEncoder = SAN_WHOLE.urlEncoder;
		...
		doSetSearchEngine: function () {
			this.searchEngine = SAN_WHOLE.searchEngine;
		},
		...
		searchComplete: function () {
			...
			if (searchResultsArr.length > 0) {
				...
				searchQueryArr = new Array(searchQuery);
...
SAN_WHOLE.namespace('SAN_WHOLE.highlightManager');
SAN_WHOLE.highlightManager = (function () {
	var _class = {
		dom: null,
		$content: null,
		init: function () {
...
SAN_WHOLE.namespace('SAN_WHOLE.tabController');
SAN_WHOLE.tabController = (function () {
	var _class = {
		DREX_SHOW_MENU: 1,
		DREX_SHOW_SEARCH: 1,
		DREX_SHOW_INDEX: 0,
...
SAN_WHOLE.namespace('SAN_WHOLE.urlEncoder');
SAN_WHOLE.urlEncoder = (function () {
	var _class = {
		urlDecoded: null,
		funcArr: [],
		utils: null,
		...
		openNextPage: function (nextpage, nextPageHref, atNewPage) {
			...
			if (vjwTutorial) {
				if (nextPageHref.length > 0) {
					var index = getPageIndex(nextPageHref);
					jwplayer().seek(drex_node_times[index]);
				}
			} else {
...
SAN_WHOLE.namespace('SAN_WHOLE.workZoneSizer');
SAN_WHOLE.workZoneSizer = (function () {
	var _class = {
		...
		doSetUrlEncoder: function () {
			var that = this;
			this._urlEncoder = SAN_WHOLE.urlEncoder;
		...
		doSetSplitter: function () {
			var that = this;
			this.splitter = SAN_WHOLE.workZoneSizer_Splitter;
		...
		doSetFrameModeSizer: function () {
			this.frameModeSizer = SAN_WHOLE.workZoneSizer_FrameMode;
...
SAN_WHOLE.namespace('SAN_WHOLE.workZoneSizer_Splitter');
SAN_WHOLE.workZoneSizer_Splitter = (function () {
	var _class = {
		$splitter: null,
		$resizable: null,
		$body: null,
...
SAN_WHOLE.namespace('SAN_WHOLE.workZoneSizer_FrameMode');
SAN_WHOLE.workZoneSizer_FrameMode = (function () {
	var _class = {
		dom: null,
		timeoutId: null,
...
SAN_WHOLE.namespace('SAN_WHOLE.navTree_Menu');
SAN_WHOLE.navTree_Menu = (function () {
	var _class = {
		$navTree: null,
		urlEncoder: null,
		dom: null,
		...
		getCurrVisibleStatusArrAsString: function () {
			...
			var nSum = menuMinimized.length>VarTOpnd.length?VarTOpnd.length:menuMinimized.length;//S202201032157
			for (var i = 0; i < nSum; i++)
				menuMinimized[i] ^= VarTOpnd[i];
			var bits = new Array();
			for (var i = 0; i < menuMinimized.length; i++)//S202201032157
		...
		populateTable: function (nodeArr, isVisible) {
			...
			for (var index = 0; index < nodeArr.length; index += 1) {
				...
				itemArr.push({
					...
					'isSelected': vjwTutorial ? drex_node_times[lastSelectIndex] == drex_node_times[index + 1] : (node.link === this.dataManager.getPageFilename()),
					'noIcon': vjwTutorial ? drex_node_links[index + 1].length == 0 : false
				});
			}
			return itemArr;
		},
...
SAN_WHOLE.namespace('SAN_WHOLE.navTree_Index');
SAN_WHOLE.navTree_Index = (function () {
	var _class = {
		$navTree: null,
		navTreeView: null,
		init: function () {
...
SAN_WHOLE.namespace('SAN_WHOLE.navTree_Search');
SAN_WHOLE.navTree_Search = (function () {
	var _class = {
		$navTree: null,
		navTreeView: null,
		init: function () {
		...
		getNavArr: function (searchResultsArr) {
			...
			for (var index = 0; index < searchResultsArr.length; index++) {
				...
				itemArr.push({
					'title': currNode[0],
					'link': retValue + '/' + currNode[1],//N2201011758
					'childs': null,
...
function initResizeArticleArticle() {
	...
	$(SAN_WHOLE.dom.$splitter).on("mousedown.splitter", function () {
...
$(document).ready(function () {
	//sunny
	var path = window['drex_file_path'];
	for (i = 0; i < SAN_WHOLE.data_menu.DREX_NODE_LINKS.length; i++) {
		SAN_WHOLE.data_menu.DREX_NODE_LINKS[i] = path + SAN_WHOLE.data_menu.DREX_NODE_LINKS[i];
	}
